...
ajoutLien.addEventListener('click', function(){
  ...
  formElt.addEventListener('submit', function(e){
    // Récupérer les elements input
    var inputElts = document.getElementsByTagName('input');
    var articleElt2 = document.createElement('article'); 
    for(var i = 0; i < inputElts.length; i++) {
      /* Ecrire dans la page */
      console.log(inputElts[i].placeholder + ' => ' + inputElts[i].value);

    }
    // Effacer le formulaire et remetre le bouton "Ajouter"
    document.getElementById('formulaire').innerHTML = ''; // vider le conteneur
    var ajouterElt = document.createElement('button');
        ajouterElt.textContent = "Ajouter un lien";
        ajouterElt.id = 'ajoutLien';
    // Ajout le bouton "Ajouter un lien" dans la page
    document.getElementById('formulaire').appendChild(ajouterElt);
